...
func TestTrimmedOrderedPeers(t *testing.T) {
	...
	_, target, pids, err := p.BestFinalized(maxPeers, 0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestBestPeer(t *testing.T) {
	...
	retRoot, retEpoch, _, err := p.BestFinalized(15, 0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestBestFinalized_ReturnsNoPeersIfNoPeersConnected(t *testing.T) {
	...
	maxPeers := 10
	p := peers.NewStatus(maxBadResponses)
	_, _, pids, _ := p.BestFinalized(maxPeers, 0)
	if len(pids) != 0 {
...
func TestBestFinalized_ReturnsMaxValue(t *testing.T) {
	...
	_, _, pids, err := p.BestFinalized(maxPeers, 0)
	if err != nil {
		t.Fatal(err)
	}
	if len(pids) != maxPeers {
